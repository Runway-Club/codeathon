...
export class ProblemEffects {
  ...
  updateProblem$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((action) =>
        setDoc(doc(this.db, 'problems', action.id), action.problem)
      ),
  ...
  getUserProblem$ = createEffect(() =>
    this.action$.pipe(
      ofType(getUserProblem),
      switchMap((action) => {
        return from(getDoc(doc(this.db, 'problems', action.userId)));
      }),
      map((action) => action.data()),
      map((data) => {
        if (data == undefined) {
          return getUserProblemFailure({ error: 'Problem not found' });
        }
        return getUserProblemSuccess({ problems: <Problem[]>data });
      }),
      catchError((error) => of(getProblemFailure({ error: error.message })))
    )
  );
  ...
  resetSubmissions$ = createEffect(() =>
    this.action$.pipe(
      ...
      catchError((error) =>
        of(resetSubmissionsFailure({ error: error.message }))
      )
    )
  );

  listingProblems$ = createEffect(() =>
    this.action$.pipe(
